...
//     process._rawDebug('init', uid, handle);
//   },
//   pre: function (uid, handle) {
//     process._rawDebug('pre', uid, handle);
//   },
//   post: function (uid, handle) {
//     process._rawDebug('post', uid, handle);
//   }
// });

process.addAsyncListener({
  create: function () {
    // process._rawDebug('current zone', Zone.current.name);
    return Zone.current;
  },

  before: function (context, storage) {
    Stack.enter(storage);
    // process._rawDebug('entering zone --->', Zone.current.name);
    // process._rawDebug(Stack);
  },

  after: function (context, storage) {
    // process._rawDebug('exiting zone <---', Zone.current.name);
    Stack.exit(storage);
  }
});
